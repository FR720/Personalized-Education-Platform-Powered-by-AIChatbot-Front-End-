      ...
      <PageTransition>
        <div className="min-h-screen bg-slate-50 dark:bg-slate-950 pt-20 px-4 sm:px-6 flex items-center justify-center relative">
          ...
          <div className="w-full max-w-md">
            <Link to="/" className="inline-flex items-center text-sm text-slate-600 dark:text-slate-400 mb-8 hover:text-blue-600 dark:hover:text-blue-400 transition-colors">
            ...
            <Card className="backdrop-blur-sm bg-white/90 dark:bg-slate-900/90 shadow-glossy border border-slate-200/50 dark:border-slate-700/50">
              <CardHeader className="text-center">
                ...
                <CardDescription>Sign up to access courses and start your learning journey</CardDescription>
              </CardHeader>
              ...
              <form onSubmit={handleSubmit}>
                <CardContent className="space-y-4">
                  ...
                  <div className="space-y-2">
                      ...
                      <div className="mt-2">
                        <div className="flex items-center justify-between mb-1">
                          <span className="text-xs text-slate-600 dark:text-slate-400 flex items-center">
                            Strength: <span className={`ml-1 font-medium ${
                    ...
                    <div className="space-y-2 mt-2">
                      <div className={`flex items-center text-xs ${password.length >= 8 ? "text-green-500" : "text-slate-500"}`}>
                        {password.length >= 8 ? (
                      ...
                      <div className={`flex items-center text-xs ${/[A-Z]/.test(password) ? "text-green-500" : "text-slate-500"}`}>
                        {/[A-Z]/.test(password) ? (
                          <CheckCircle className="h-3 w-3 mr-1.5" />
                        ) : (
                          <div className="h-3 w-3 rounded-full border border-current mr-1.5"></div>
                      ...
                      <div className={`flex items-center text-xs ${/[0-9]/.test(password) ? "text-green-500" : "text-slate-500"}`}>
                        {/[0-9]/.test(password) ? (
                          <CheckCircle className="h-3 w-3 mr-1.5" />
                        ) : (
                          <div className="h-3 w-3 rounded-full border border-current mr-1.5"></div>
                ...
                <CardFooter className="flex flex-col">
                  ...
                  <div className="mt-4 text-xs text-slate-500 dark:text-slate-400 text-center">
                    ...
                    <Link to="/terms" className="text-blue-600 dark:text-blue-400 hover:underline mx-1">Terms of Service</Link>
                    and
                    <Link to="/privacy" className="text-blue-600 dark:text-blue-400 hover:underline ml-1">Privacy Policy</Link>.
                  </div>
