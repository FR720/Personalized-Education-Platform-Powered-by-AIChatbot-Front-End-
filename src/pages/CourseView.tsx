...
import { 
  Play, Pause, BookOpen, Check, Clock, MessageCircle, ChevronLeft, 
  ChevronRight, Video, FileText, Settings, CheckCircle, Award
} from "lucide-react";
...
const courseData = {
  ...
  description: "Learn the fundamentals of machine learning algorithms and applications in this comprehensive course.",
  progress: 65,
  ...
  image: "https://images.unsplash.com/photo-1488590528505-98d2b5aba04b?auto=format&fit=crop&q=80&w=1200&h=400",
  sections: [
    {
      id: 1,
...
const totalModules = courseData.sections.reduce((acc, section) => acc + section.modules.length, 0);
const completedModules = courseData.sections.reduce((acc, section) => {
  const completedInSection = section.modules.filter(module => module.completed).length;
...
const CourseView = () => {
  ...
  const [activeModule, setActiveModule] = useState(courseData.sections[2].modules[0]);
  const [videoPlaying, setVideoPlaying] = useState(false);
  const [chatMessage, setChatMessage] = useState("");
  const [chatHistory, setChatHistory] = useState<Array<{ sender: string; message: string }>>([
    { sender: "ai", message: "Hello! I'm your course assistant. Need help with any concepts?" }
  ]);
  ...
  const handleSendMessage = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      setChatHistory(prev => [...prev, { sender: "ai", message: randomResponse }]);
      setChatMessage("");
    ...
    <>
      ...
      <PageTransition>
        <div className="min-h-screen bg-slate-50 dark:bg-slate-950 pt-16 pb-12">
          {/* Course header */}
          <div 
            className="relative aspect-[3/1] min-h-[220px] bg-slate-800 overflow-hidden"
          >
            <img 
            ...
            <div className="absolute inset-0 flex items-end">
              <div className="container max-w-7xl mx-auto px-4 sm:px-6 pb-8">
                ...
                <div className="flex flex-wrap items-center gap-3 text-white/90 text-sm">
                  <Badge variant="secondary" className="bg-white/20 hover:bg-white/25 text-white border-none">
                    {courseData.category}
                  </Badge>
                  ...
                  <div className="flex items-center">
                    <CheckCircle className="h-4 w-4 mr-1" /> {completedModules}/{totalModules} completed
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="container max-w-7xl mx-auto px-4 sm:px-6 py-6">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 space-y-6">
                ...
                <div 
                  ref={videoRef}
                  className={`bg-slate-900 rounded-xl overflow-hidden aspect-video relative group opacity-0 ${isLoaded ? "animate-fade-in" : ""}`}
                    ...
                    {activeModule.type === 'video' ? (
                      <>
                        ...
                        <div className="absolute bottom-0 inset-x-0 p-4 flex items-center justify-between text-white">
                          <div className="flex items-center">
                            <h3 className="text-lg font-medium">{activeModule.title}</h3>
                          </div>
                          <Button 
                            variant="ghost" 
                            size="sm"
                      ...
                      <div className="text-center p-8">
                        ...
                        <h3 className="text-xl font-semibold text-white mb-2">{activeModule.title}</h3>
                        <p className="text-white/70 mb-6">This quiz contains {activeModule.questions} questions</p>
                        <Button>
                          Start Quiz
                        </Button>
                      </div>
                      ...
                      <div className="text-center p-8">
                        ...
                        <h3 className="text-xl font-semibold text-white mb-2">{activeModule.title}</h3>
                        <p className="text-white/70 mb-6">Reading material - {activeModule.pages} pages</p>
                        <Button>
                          ...
                          <div key={section.id} className="border-b border-slate-200 dark:border-slate-800 last:border-b-0">
                            <div 
                              className={`p-4 flex items-center justify-between transition-colors ${
                                section.completed ? "bg-blue-50/50 dark:bg-blue-900/10" : ""
                              }`}
                            >
                              <div className="flex items-center">
                                ...
                                <div>
                                  ...
                                  <div className="text-sm text-slate-500 dark:text-slate-400 flex items-center mt-0.5">
                                    <Clock className="h-3.5 w-3.5 mr-1" /> {section.duration}
                              ...
                              <Badge variant={section.completed ? "secondary" : "outline"}>
                              ...
                              {section.modules.map(module => (
                                <button
                                  ...
                                  className={`w-full text-left p-3 border-b border-slate-100 dark:border-slate-800 last:border-b-0 flex items-start group transition-colors hover:bg-slate-100/60 dark:hover:bg-slate-800/50 ${
                                    activeModule.id === module.id ? "bg-blue-50 dark:bg-blue-900/10" : ""
                                  }`}
                                >
                                  ...
                                  <div>
                                    <div className="font-medium text-sm text-slate-900 dark:text-white">{module.title}</div>
                                    <div className="text-xs text-slate-500 dark:text-slate-400 flex items-center mt-1">
                                      {module.type === 'video' ? (
                                        <>
                                          <Video className="h-3 w-3 mr-1" /> {module.duration} video
                                        </>
                                      ) : module.type === 'quiz' ? (
                                        <>
                                          <FileText className="h-3 w-3 mr-1" /> {module.questions} question quiz
                                          {module.completed && <span className="ml-2 text-green-500">Score: {module.score}%</span>}
                                        </>
                                      ) : (
                                        <>
                                          <BookOpen className="h-3 w-3 mr-1" /> {module.pages} pages reading
                                        </>
                                      )}
                                    </div>
                                  </div>
                          ...
                          <div className="space-y-4">
                            <div className="flex items-start space-x-4">
                              ...
                              <div className="flex-1 bg-white dark:bg-slate-900 rounded-lg p-4 shadow-sm border border-slate-200 dark:border-slate-800">
                                <div className="font-medium text-slate-900 dark:text-white mb-1">Instructor</div>
                                <p className="text-slate-600 dark:text-slate-300">
                                  Welcome to the course discussion! Feel free to ask questions about any of the course material here.
                                </p>
                              </div>
                            </div>
                            ...
                            <div className="flex items-start space-x-4">
                              ...
                              <div className="flex-1 bg-white dark:bg-slate-900 rounded-lg p-4 shadow-sm border border-slate-200 dark:border-slate-800">
                                <div className="font-medium text-slate-900 dark:text-white mb-1">Alex</div>
                                <p className="text-slate-600 dark:text-slate-300">
                                  Could someone explain the difference between supervised and unsupervised learning again?
                                </p>
                              </div>
                            </div>
                            ...
                            <div className="flex items-start space-x-4">
                              <div className="h-10 w-10 rounded-full bg-slate-100 dark:bg-slate-800"></div>
                              <div className="flex-1 bg-white dark:bg-slate-900 rounded-lg p-4 shadow-sm border border-slate-200 dark:border-slate-800">
                                <div className="font-medium text-slate-900 dark:text-white mb-1">Jordan</div>
                                <p className="text-slate-600 dark:text-slate-300">
                                  Supervised learning uses labeled data where the algorithm learns the mapping between inputs and outputs. Unsupervised learning works with unlabeled data and tries to find patterns or structure in the data on its own.
                                </p>
                              </div>
                            </div>
                      ...
                      <Card>
                        <CardContent className="pt-6">
                          <div className="bg-yellow-50 dark:bg-yellow-900/10 border border-yellow-200 dark:border-yellow-900/50 rounded-lg p-4 mb-4">
                            <h3 className="text-sm font-medium text-yellow-800 dark:text-yellow-500 mb-1">Supervised Learning Notes</h3>
                            <p className="text-sm text-yellow-700 dark:text-yellow-300">
                              Classification vs. Regression: Classification predicts categories, regression predicts continuous values. Common algorithms: Decision Trees, SVM, Linear Regression.
                            </p>
                          </div>
              ...
              <div className="space-y-6">
                {/* Course info */}
                <Card className={`opacity-0 ${isLoaded ? "animate-fade-in" : ""}`} style={{ animationDelay: "400ms" }}>
                  <CardContent className="pt-6">
                    <h2 className="text-xl font-semibold text-slate-900 dark:text-white mb-4">Course Overview</h2>
                    
                    <div className="space-y-4">
                      <div className="flex justify-between items-center">
                        <h3 className="text-sm font-medium text-slate-500 dark:text-slate-400">Progress</h3>
                        <span className="text-sm font-medium text-slate-900 dark:text-white">{courseData.progress}%</span>
                      </div>
                      ...
                      <div className="pt-2">
                        <div className="flex justify-between text-sm py-2 border-b border-slate-100 dark:border-slate-800">
                          ...
                          <span className="font-medium text-slate-900 dark:text-white">{courseData.instructor}</span>
                        </div>
                        <div className="flex justify-between text-sm py-2 border-b border-slate-100 dark:border-slate-800">
                          <span className="text-slate-600 dark:text-slate-400">Modules</span>
                          <span className="font-medium text-slate-900 dark:text-white">{totalModules}</span>
                        </div>
                        <div className="flex justify-between text-sm py-2 border-b border-slate-100 dark:border-slate-800">
                          <span className="text-slate-600 dark:text-slate-400">Completed</span>
                          <span className="font-medium text-slate-900 dark:text-white">{completedModules}</span>
                        </div>
                        <div className="flex justify-between text-sm py-2 border-b border-slate-100 dark:border-slate-800">
                          <span className="text-slate-600 dark:text-slate-400">Enrolled</span>
                          <span className="font-medium text-slate-900 dark:text-white">{courseData.enrollmentDate}</span>
                        </div>
                      </div>
                    </div>
                ...
                <Card className={`overflow-hidden h-[400px] flex flex-col opacity-0 ${isLoaded ? "animate-fade-in" : ""}`} style={{ animationDelay: "500ms" }}>
                  <div className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white p-4">
                    <div className="flex items-center">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-blue-100">Ask questions about course material</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="p-4 border-t border-slate-200 dark:border-slate-800">
                    <form onSubmit={handleSendMessage} className="flex space-x-2">
                      ...
                      />
                      <Button type="submit" size="sm" disabled={!chatMessage.trim() || sendingMessage}>
