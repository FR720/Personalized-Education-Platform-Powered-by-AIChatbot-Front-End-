...
import { BookOpen, Clock, BarChart, Play, PlusCircle, MessageCircle, Book, Video, Award, CheckCircle, TrendingUp } from "lucide-react";
...
const Dashboard = () => {
  ...
  const [chatHistory, setChatHistory] = useState<Array<{ sender: string; message: string }>>([
    { sender: "ai", message: "Hello! I'm your AI learning assistant. How can I help you with your courses today?" }
  ]);
          ...
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <div className="lg:col-span-2 space-y-6">
                {/* Stats cards */}
                <div 
                  className={`grid grid-cols-1 md:grid-cols-3 gap-6 opacity-0 ${isLoaded ? "animate-slide-up" : ""}`}
                ...
                >
                  <div className="flex items-center justify-between mb-5">
                    <h2 className="text-xl font-semibold text-slate-900 dark:text-white">Continue Learning</h2>
                    <Link to="/courses">
                        ...
                        <div className="aspect-[16/9] bg-slate-100 dark:bg-slate-800 relative overflow-hidden">
                          ...
                          <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent">
                            <div className="absolute bottom-4 left-4">
                              <span className="px-2 py-1 bg-blue-600/90 text-white text-xs rounded-full">{course.category}</span>
                            </div>
                          </div>
                        </div>
                ...
                >
                  <Tabs defaultValue="recommendations" className="w-full">
                    <TabsList className="grid w-full grid-cols-2 mb-6">
                      <TabsTrigger value="recommendations">Recommended Courses</TabsTrigger>
                        ...
                        <Card key={course.id} className="overflow-hidden">
                          <div className="flex flex-col md:flex-row">
                            ...
                            <div className="p-5 md:w-2/3 flex flex-col">
                              <div className="mb-2">
                                <span className="px-2 py-1 bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 text-xs rounded-full">{course.category}</span>
                              </div>
                              <h3 className="text-lg font-semibold text-slate-900 dark:text-white mb-2">{course.title}</h3>
                              <p className="text-slate-600 dark:text-slate-400 text-sm flex-grow mb-4">{course.description}</p>
                              ...
                              <div className="flex items-center justify-between mt-auto">
                                ...
                                <Link to={`/course/${course.id}`} className="text-blue-600 dark:text-blue-400 text-sm hover:underline">
                                  Preview
                                </Link>
                              </div>
                            </div>
                    ...
                    <TabsContent value="achievements" className="space-y-4">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        {[
                          { 
                            title: "Fast Learner", 
                            description: "Completed 3 course modules in a single day", 
                          ...
                          <Card key={index} className={`overflow-hidden ${!achievement.unlocked && "opacity-50"}`}>
                            <CardHeader className="pb-2 flex flex-row items-center space-x-4">
                              <div className={`p-2 rounded-full ${achievement.unlocked ? "bg-amber-100" : "bg-slate-100"}`}>
                                {achievement.icon}
                              </div>
                              <div>
                            ...
                            <CardFooter className="pt-1">
                              <span className={`text-xs font-medium ${achievement.unlocked ? "text-green-500" : "text-slate-500"}`}>
              ...
              <div 
                className={`space-y-6 opacity-0 ${isLoaded ? "animate-slide-up" : ""}`}
                style={{ animationDelay: "300ms" }}
              >
                ...
                <Card className="overflow-hidden h-[500px] flex flex-col">
                  <CardHeader className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white pb-4">
                    <div className="flex items-center">
                      ...
                      <div>
                        ...
                        <CardDescription className="text-blue-100">Get personalized help and recommendations</CardDescription>
                      </div>
                    </div>
                  </CardHeader>
                  ...
                  <CardFooter className="p-4 border-t border-slate-200 dark:border-slate-800">
                    <form onSubmit={handleSendMessage} className="w-full flex space-x-2">
                      ...
                      />
                      <Button type="submit" disabled={!chatMessage.trim() || sendingMessage} size="sm">
                        Send
                      </Button>
                    </form>
                  </CardFooter>
                      ...
                      <div className="relative h-full w-full">
                            ...
                            <div key={i} className="flex flex-col items-center w-full">
                              ...
                              >
                                ...
                                <div 
                                  ...
                                  style={{ height: `${(item.hours / 5) * 150}px` }}
                                ></div>
                        ...
                        <div className="absolute inset-y-0 left-0 flex flex-col justify-between pointer-events-none">
                          {[5, 4, 3, 2, 1, 0].map((hour) => (
                            <div key={hour} className="text-xs text-slate-400 -translate-y-1/2">
                              {hour}h
                            </div>
                          ))}
                        </div>
